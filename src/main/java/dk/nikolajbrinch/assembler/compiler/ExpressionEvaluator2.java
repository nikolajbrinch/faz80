...
public class ExpressionEvaluator2 implements ExpressionVisitor<Evaluated> {
  ...
  public Evaluated visitStringExpression(StringExpression expression) {
    if (expression.stringValue().canBeNUmber()) {
      return Evaluated.of(expression.stringValue().asNumberValue(), expression.stringValue().asNumberValue().size());
    }
